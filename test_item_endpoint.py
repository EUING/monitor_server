...
def test_change_item_name(api):
    change_name_info = {"old_name": "새 폴더/test.jpg", "new_name": "새 폴더/test.bmp"}
...
def test_insert_file_info(api):
    ...
    resp = api.post("/file/info", data = json.dumps(file_info), content_type = "application/json")
...
def test_modify_file_info(api):
    file_info = {"name": "새 폴더/test.jpg", "size": 1000000, "last_modified_time": "2021-06-24 21:35:00"}
    resp = api.patch("/file/info/test.jpg", data = json.dumps(file_info), content_type = "application/json")
...
def test_insert_folder_info(api):
    folder_info = {"name": "testtest", "creation_time": "2021-06-28 23:00:00"}
    resp = api.post("/folder/info", data = json.dumps(folder_info), content_type = "application/json")
